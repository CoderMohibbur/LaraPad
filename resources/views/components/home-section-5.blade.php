...
<section id="approach-leadgen" class="relative border-y-2 bg-gray-50 dark:bg-gray-900 overflow-hidden">
  ...
  <div class="relative max-w-screen-xl mx-auto px-6 lg:px-8 py-12">
    ...
    <div class="mt-12 grid lg:grid-cols-3 gap-8 items-start">
      ...
      <!-- Right: Accordion (DETAILS/SUMMARY based) -->
      <!-- একটাই ওপেন রাখতে চাইলে নিচের div-এ data-single যোগ করে দিন -->
      <div class="order-1 lg:order-2 lg:col-span-2">
        <div id="lg-accordion" class="space-y-4" data-single>
          <!-- Item 1 (open) -->
          <details class="acc-item border border-gray-200 dark:border-gray-700 rounded-xl overflow-hidden" id="faq-1" open>
            <summary class="acc-summary ripple flex items-center justify-between w-full px-6 py-4 text-base md:text-lg font-semibold text-gray-800 dark:text-gray-200 bg-gray-100 dark:bg-gray-800 cursor-pointer list-none select-none">
              <span>ICP Research &amp; Laser-Targeted List Building</span>
              <svg class="chev h-4 w-4 transition-transform duration-300" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" d="M19 9l-7 7-7-7"/></svg>
            </summary>
            <div class="acc-content bg-white dark:bg-gray-900">
          ...
          <!-- Item 2 -->
          <details class="acc-item border border-gray-200 dark:border-gray-700 rounded-xl overflow-hidden" id="faq-2">
            <summary class="acc-summary ripple flex items-center justify-between w-full px-6 py-4 text-base md:text-lg font-semibold text-gray-800 dark:text-gray-200 bg-gray-100 dark:bg-gray-800 cursor-pointer list-none select-none">
          ...
          <!-- Item 3 -->
          <details class="acc-item border border-gray-200 dark:border-gray-700 rounded-xl overflow-hidden" id="faq-3">
            <summary class="acc-summary ripple flex items-center justify-between w-full px-6 py-4 text-base md:text-lg font-semibold text-gray-800 dark:text-gray-200 bg-gray-100 dark:bg-gray-800 cursor-pointer list-none select-none">
          ...
          <!-- Item 4 -->
          <details class="acc-item border border-gray-200 dark:border-gray-700 rounded-xl overflow-hidden" id="faq-4">
            <summary class="acc-summary ripple flex items-center justify-between w-full px-6 py-4 text-base md:text-lg font-semibold text-gray-800 dark:text-gray-200 bg-gray-100 dark:bg-gray-800 cursor-pointer list-none select-none">
          ...
          <!-- Item 5 -->
          <details class="acc-item border border-gray-200 dark:border-gray-700 rounded-xl overflow-hidden" id="faq-5">
            <summary class="acc-summary ripple flex items-center justify-between w-full px-6 py-4 text-base md:text-lg font-semibold text-gray-800 dark:text-gray-200 bg-gray-100 dark:bg-gray-800 cursor-pointer list-none select-none">
  ...
  <style>
    ...
    .ripple:active::after{left:var(--x);top:var(--y);opacity:1;width:220px;height:220px;transition:width .45s,height .45s,opacity .9s;opacity:0}
    ...
    /* Smooth open/close (CSS-only, robust) */
    .acc-content{ max-height:0; overflow:hidden; transition:max-height .35s ease; }
    .acc-item[open] > .acc-content{ max-height: 900px; } /* যথেষ্ট বড় রাখুন */
  ...
  <script>
    // One-open-only behavior (optional): #lg-accordion এ data-single থাকলে কার্যকর
    (function(){
      const wrap = document.getElementById('lg-accordion');
      if(!wrap || !wrap.hasAttribute('data-single')) return;
      wrap.addEventListener('toggle', (e)=>{
        const current = e.target;
        if(!(current instanceof HTMLDetailsElement) || !current.open) return;
        wrap.querySelectorAll('details[open]').forEach(d => { if(d !== current) d.removeAttribute('open'); });
      });
      // ripple coords on summary
      wrap.addEventListener('pointerdown', (e)=>{
        const s = e.target.closest('.acc-summary'); if(!s) return;
    ...
    // Count-up metrics on view (unchanged)
    (function(){
