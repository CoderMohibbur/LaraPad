<x-app-layout>
    <div class="max-w-7xl mx-auto p-6">
        <h2 class="text-2xl font-bold text-gray-800 dark:text-white mb-6">✏️ Edit Testimonial</h2>
        ...
        <form action="{{ route('testimonials.update', $testimonial) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="grid md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name', $testimonial->name) }}"
                        class="mt-1 block w-full rounded-md shadow-sm border-gray-300 dark:border-gray-600 dark:bg-gray-800 dark:text-white" required>
                    @error('name')
                ...
                <div>
                    ...
                    @if($testimonial->image)
                        <div class="mt-3">
                            ...
                            <img src="{{ asset($testimonial->image) }}" alt="Current Image" class="w-32 h-32 object-cover rounded border">
                        </div>
                    @endif
                </div>
                ...
                <!-- Serial -->
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Serial</label>
                    <input type="number" name="serial" value="{{ old('serial', $testimonial->serial) }}" min="1"
                        class="mt-1 block w-full rounded-md shadow-sm border-gray-300 dark:border-gray-600 dark:bg-gray-800 dark:text-white">
                    @error('serial')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
            ...
            <div>
                ...
                <textarea name="quote" rows="2"
                    class="mt-1 block w-full rounded-md shadow-sm border-gray-300 dark:border-gray-600 dark:bg-gray-800 dark:text-white">{{ old('quote', $testimonial->quote) }}</textarea>
                @error('quote')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <textarea name="description" rows="4"
                    class="mt-1 block w-full rounded-md shadow-sm border-gray-300 dark:border-gray-600 dark:bg-gray-800 dark:text-white">{{ old('description', $testimonial->description) }}</textarea>
                @error('description')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
